...
#--------------- Added by KC ---------------#
import models.Scalable_MNIST as mnist_model                 
#-------------------------------------------#
...
#---------------------------------------------------- Added by KC ----------------------------------------------------#
    # A self-defined code which replaces initiated parameters with weights from trained model, added by Kai Cheng
    # Different Models that supports dynamic can be added by including the necessary library and switching the models
    #Just Change the model, logs and the ratio according to different models
    
    #This Slice of code initiates "Prev_Model" with the desired Model, similar to the structure in the original
    if True:
        LOGGER_INFO = "logs/2024.08.23-150832/checkpoint.pth.tar"
        RATIO = 16
        Prev_Model = apputils.load_lean_checkpoint(mnist_model.mnist_25(), LOGGER_INFO,
                                              model_device=args.device)
